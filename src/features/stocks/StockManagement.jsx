                    ...
                    {isAdmin && (
                        <>
                            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-4">
                                <input type="text" name="name" value={newStock.name} onChange={handleChange} placeholder="Item name" className="input" />
                                <input type="text" name="category" value={newStock.category} onChange={handleChange} placeholder="Category" className="input" />
                                <input type="number" name="quantity" value={newStock.quantity} onChange={handleChange} placeholder="Quantity" className="input" />
                                <input type="number" name="price" value={newStock.price} onChange={handleChange} placeholder="Price" className="input" />
                            </div>
                            <div className="mb-4">
                                {editingIndex !== null ? (
                                    <button className="btn btn-blue" onClick={handleSave}>Save Changes</button>
                                ) : (
                                    <button className="btn btn-green" onClick={handleAdd}>Add Item</button>
                                )}
                            </div>
                        </>
                    )}
                                        ...
                                        <tr key={item._id} className="border-t">
                                            ...
                                            {isAdmin && (
                                                <td className="px-4 py-2 text-sm space-x-2">
                                                    <button className="btn btn-yellow" onClick={() => handleEdit(index)}>Edit</button>
                                                    <button className="btn btn-red" onClick={() => handleDelete(index)}>Delete</button>
                                                </td>
                                            )}
                                        </tr>
                                    ))
